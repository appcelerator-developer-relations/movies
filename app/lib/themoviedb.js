...
theMovieDb.common = {
  ...
  validateCallbacks: function (callbacks) {
    ...
    if (typeof callbacks[0] !== 'function' || typeof callbacks[1] !== 'function') {
      Ti.API.error('Success and error parameters must be functions!');
    }
  },
  validateRequired: function (args, argsReq, opt, optReq, allOpt) {
    ...
    if (args.length !== argsReq) {
      Ti.API.error('The method requires  ' + argsReq + ' arguments and you are sending ' + args.length + '!');
    }
    ...
    if (argsReq > 2) {
      for (i = 0; i < optReq.length; i += 1) {
        if (!opt.hasOwnProperty(optReq[i])) {
          Ti.API.error(optReq[i] + ' is a required parameter and is not present in the options!');
        }
      }
    }
  },
